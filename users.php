...
if (isset($_GET['username'])) {
       if (DB::query('SELECT username FROM users WHERE username=:username', array(':username'=>$_GET['username']))) {
				...
				//['postimg']['size']['town']['estate']['rent']['rent_decribe']['posted_at']
...
<head>
    ...
    <title >Talnet| Users</title>  <link href="assets/img/avatar.png" rel="shortcut icon" sizes="250x250"> 
...
<div class="searchbox">
 <form method="post" action="search.php?town=$town&estate=$estate" id="searching"><input hidden value="" type="text" name="estate"  id="estate" >
       ...
       <input hidden type="button" class="post2" style=" border: 0; color:white; font-weight: bold;  padding: 2px 4px; background-color:#228B22;">
        <input type="submit" value="Search &#128269;"class="post" style=" border: 0; color:white; font-weight: bold; float: left; padding: 2px 4px; background-color:#228B22;"></form>
    ...
    $(window).scroll(function() {
            if ($(this).scrollTop() + 1 >= $('body').height() - $(window).height()) {
                    if (working == false) {
                        ...
                        type: "GET",
                        url: "api/profilepostsusers?username=<?php echo $username; ?>&start="+start,
                        processData: false,
                      ...
                      success: function(r) {
                                ...
                                $.each(posts, function(index) {
                                        ...
                                        if (posts[index].PostImage == "") {
                                                $('.timelineposts').html(  
                                                $('.timelineposts').html() + '<blockquote><p><font size="2"><font size="2"><p><font color="red">Residence: </font> '+posts[index].Residence+ '</p><font size="1"><i><font color="grey"><div style="text-align:right">POSTID: </font> '+posts[index].PostId+ '</i></p></font></div><font color="red">House size: </font> '+posts[index].PlotSize+ ' <font color="red">  Rent: </font> '+posts[index].Rentamount+'</p><p><font color="red">Town: </font> '+posts[index].Town+'  <font color="red">Estate: </font> '+posts[index].Estate+'</p><p><font color="red">Phone number: </font> '+posts[index].PhoneNumber+ '   <button   class="phonecell" type="button" style="width:20%;background-image:url(&quot;none&quot;);background-color:green;color:#fff;margin:0px 0px 6px;border:none;box-shadow:none;text-shadow:none;opacity:0.9;text-transform:uppercase;font-weight:bold;font-size:13px;letter-spacing:0.4px;line-height:1;outline:none;" data-call="'+posts[index].PhoneNumber+'">call</button></p><p><font color="grey">Posted by ~ </font> '+posts[index].Postedby+'</p></font><footer>Description:'+posts[index].describe+'</p> Posted at '+posts[index].PostedAt+'</footer></blockquote>'
                                        )
    ...
    return true;
       ...
       $(document).ready(function() {
                $.ajax({
                        ...
                        type: "GET",
                        url: "api/profilepostsusers?username=<?php echo $username; ?>&start=0",
                        processData: false,
                        contentType: "application/json",
                        data: '',
                      success: function(r) {
